...
class HTranslatorPrivate : public QObjectPrivate
{
    Q_DECLARE_PUBLIC(HTranslator)
public:
    ...
    QList<HTranslator*> subTranslators;
    ...
    QString getSourceText(const QString translatedText) const;
};
...
/*!
    ...
    \brief The HTranslator class provides internationalization support for text
    output.

    \ingroup i18n

    An object of this class contains a set of translations from a
    source language to a target language. HTranslator provides
    functions to look up translations in a translation file.
    Translation files are created using \l{Qt Linguist}.

    The most common use of HTranslator is to: load a translation
...
HTranslator::HTranslator(QObject * parent)
//    : QObject(*new HTranslatorPrivate, parent)
    : QTranslator(parent)
{
}
...
/*!
    ...
    Usually, it is better to use the HTranslator::load(const QLocale &,
...
bool HTranslator::load(const QString & filename, const QString & directory,
...
bool HTranslatorPrivate::do_load(const QString &realname, const QString &directory)
{
    HTranslatorPrivate *d = this;
    bool ok = false;
...
/*!
    ...
    On operating systems where file system is case sensitive, HTranslator also
    tries to load a lower-cased version of the locale name.
*/
bool HTranslator::load(const QLocale & locale,
                       const QString & filename,
                       const QString & prefix,
                       const QString & directory,
                       const QString & suffix)
...
*/
bool HTranslator::load(const uchar *data, int len, const QString &directory)
{
    Q_D(HTranslator);
    d->clear();
...
bool HTranslatorPrivate::do_load(const uchar *data, qsizetype len, const QString &directory)
{
    bool ok = true;
    ...
    while (data < end - 5) {
        ...
        if (tag == HTranslatorPrivate::Language) {
            language = QString::fromUtf8((const char*)data, blockLen);
        } else if (tag == HTranslatorPrivate::Contexts) {
            contextArray = data;
            contextLength = blockLen;
        } else if (tag == HTranslatorPrivate::Hashes) {
            offsetArray = data;
            offsetLength = blockLen;
        } else if (tag == HTranslatorPrivate::Messages) {
            messageArray = data;
            messageLength = blockLen;
        } else if (tag == HTranslatorPrivate::NumerusRules) {
            numerusRulesArray = data;
            numerusRulesLength = blockLen;
        } else if (tag == HTranslatorPrivate::Dependencies) {
    ...
    if (ok) {
        ...
        for (int i = 0 ; i < dependenciesCount; ++i) {
            HTranslator *translator = new HTranslator;
...
QString HTranslatorPrivate::do_translate(const char *context, const char *sourceText,
                                         const char *comment, int n) const
{
    ...
    /*
        Check if the context belongs to this HTranslator. If many
...
searchDependencies:
    for (HTranslator *translator : subTranslators) {
...
static QString getSource(const uchar *m, const uchar *end, const QString translatedText)
{
//    qDebug() << "find str: " << translatedText;
    for (;;) {
        uchar tag = 0;
        if (m < end)
            tag = read8(m++);
        ...
        case Tag_Translation: {
            ...
            m += 4;
            QString str(len / 2, Qt::Uninitialized);
            qFromBigEndian<ushort>(m, str.length(), str.data());

            if (str != translatedText) {
                return QString();
            }
            m += len;
            continue;
        ...
        case Tag_SourceText: {
            quint32 len = read32(m);
            m += 4;
            QString str = QString::fromLocal8Bit((char*)m, len);
//            qDebug() << "source is: " << str;
            return str;
        }
...
QString HTranslatorPrivate::getSourceText(const QString translatedText) const {
    qDebug() << "shko test" << __FUNCTION__;
    ...
    if (start <= end) {
        // TODO: find appropriate start point
        while (start < offsetArray + offsetLength) {
            start += 4;
            quint32 ro = read32(start);
            start += 4;
            QString tn = getSource(messageArray + ro, messageArray + messageLength, translatedText);
            if (!tn.isNull()) {
                return tn;
            }
        }
...
/*!
    ...
    HTranslator, this function can be reimplemented.

    \sa load()
*/
QString HTranslator::translate(const char *context, const char *sourceText, const char *disambiguation,
                               int n) const
{
    Q_D(const HTranslator);
...
*/
bool HTranslator::isEmpty() const
{
    Q_D(const HTranslator);
 ...
 */
QString HTranslator::language() const
{
    Q_D(const HTranslator);
    return d->language;
}
...
/*!
    ...
    from a file.
 */
QString HTranslator::filePath() const
{
    Q_D(const HTranslator);
    return d->filePath;
}
...
QString HTranslator::getSourceText(const QString translatedText) {
    Q_D(const HTranslator);
    return d->getSourceText(translatedText);
}

QT_END_NAMESPACE

#include "moc_htranslator.cpp"
